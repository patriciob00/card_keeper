...
class CardWithRippleAndFlip extends StatefulWidget {
  const CardWithRippleAndFlip(
      ...
      this.width, // opcional: controla o tamanho
      this.aspectRatio = 63 / 88 // proporção padrão de carta (~0.716)
      });
...
class _CardWithRippleAndFlipState extends State<CardWithRippleAndFlip>
  ...
  void onTapCard() {
    print('calling onTapCard aqui');
    if (widget.onTap != null) {
      widget.onTap!();
    } else {
      print('calling flipCard aqui');
      flipCard();
    }
  }
  ...
  bool isFrontImage(double angle) {
    ...
    print('angle ${angle <= degrees90 || angle >= degrees270}');
    return angle <= degrees90 || angle >= degrees270;
  }

  @override
  Widget build(BuildContext context) {
    final double cardW = widget.width ?? MediaQuery.of(context).size.width * 0.6;
    final double cardH = cardW / widget.aspectRatio;
...
class CardBackInfo extends StatelessWidget {
  const CardBackInfo(
      {super.key, this.currentCard, this.isAlreadyOnList = false});
